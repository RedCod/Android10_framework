...
public class Feedback {
	 ...
	 */
	public String addFeedback(int accountId,String content,String fdDateTime) {
		String response = "{\"AddFeedback\":[{ \"response\":false}]}";
		String sql = "INSERT INTO tblFeedback(AccountId,Content,FdDateTime)VALUES('" + accountId + "','" + content + "','"+ fdDateTime +"')";
	 ...
	 */
	public String editFeedback(int feedbackId,String content,String fdDateTime) {
		String response = "{\"EditFeedback\":[{ \"response\":false}]}";
		String sql = "UPDATE tblFeedback SET Content='" + content + "',FdDateTime='" + fdDateTime + "' WHERE Id='" + feedbackId + "'";
		Database database = new Database();
		try {
			database.connect();
			int s = database.execSQL(sql);
	 ...
	 * @param feedbackId //which feedback.
	 * @return			//return Item as JSON.
	 */
	public String getItem(int feedbackId) {
		String query_sql = "SELECT *FROM tblFeedback WHERE Id='" + feedbackId + "'";
		Database database = new Database();
		String content = "{\"Item\":[\n";
		try {
			database.connect();
	 ...
	 */
	public String remove(int feedbackId) {
		String response = "{\"Remove\":[{ \"response\":false}]}";
		String sql = "DELETE FROM tblFeedback WHERE Id='" + feedbackId +"'";
		Database database = new Database();
		try {
			database.connect();
			int s = database.execSQL(sql);
