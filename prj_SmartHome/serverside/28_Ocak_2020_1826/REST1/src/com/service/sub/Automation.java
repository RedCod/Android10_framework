...
///ADD:@{
	...
	public String addAction(int accountId,
		...
		String insert_sql = "INSERT INTO tblAutomationOperations(AccountId,AutomationId,ActionType,DeviceId,DeviceSwitch,AutomationIdAssign,TimeLapseValue,Command,ItemSort)"
...
///EDiT:@{
	 ...
	 */
	public String editAutomation(int accountId,
								 String accountName,
			                     int automationId,
								 String automationName,
								 int coverImage,
	 ...
	 */
	public String activateAutomation(int accountId,
			                         String accountName,
			                         int automationId) {
		...
		///for improving security[28120201126]:@}
        int s = setActivateOrDeactivateAutomation(automationId,1);
        if(s > 0)
	 ...
	 * @return			   //return true(as JSON) if successful.
	 */
	public String deactivateAutomation(int accountId,
			 						   String accountName,
			 						   int automationId) {
		String response = "{\"DeactivateAutomation\":[{ \"response\":false}]}";
		///for improving security[28120201126]:@{
		if(!Account.accountIsExists(accountId, accountName)){
			return response;
		}
		///for improving security[28120201126]:@}
        int s = setActivateOrDeactivateAutomation(automationId,0);
        if(s > 0)
			response = "{\"DeactivateAutomation\":[{ \"response\":true}]}";
		return response;
	 ...
	 */
	public String editAutomationItemSort(int accountId,
			                             String accountName,
										 int automationId,
										 int itemSort) {
		String response = "{\"EditAutomation\":[{ \"response\":false}]}";
	 ...
	 */
	public String editCondition(int accountId,
								String accountName,
			                    int conditionId,
								String conditionType,
								String conditionValue,
	 ...
	 * @param accountId				//which account
	 * @param accountName			//accunt(email_or_phone)
	 ...
	 * @return						//return true(as JSON) if action add to "tblAutomationOperation" table.
	 */
	public String editAction(int accountId,
			 				 String accountName,
			                 int actionId,
			                 String deviceVirtualAddress,
			                 String deviceSwitch,
		...
		///for improving security[28120201126]:@}
		String produce_command = "/"+deviceVirtualAddress+"/" +deviceSwitch; //exp: "/devicevirtualaddress/ON"
		String update_sql = "UPDATE tblAutomationOperations SET DeviceSwitch='" +deviceSwitch+"',AutomationIdAssign='"+automationIdAssign+"',TimeLapseValue='"+timeLapseValue+"',Command='" + produce_command+"',ItemSort='"+itemSort+"' WHERE Id='"+actionId+"'";
		Database database = new Database();
		try {
			database.connect();
			int s = database.execSQL(update_sql);
...
///GET:@{
	 ...
	 */
	public String getAllAutomationsAsList(int accountId,String accountName,int familyId) {
		
		///for improving security[28120201126]:@{
		if(!Account.accountIsExists(accountId, accountName)){
			return "{\"AllAutomationList\":[ ]}";
		}
		///for improving security[28120201126]:@}
	 ...
	 */
	public String getAllConditionsAsList(int accountId,String accountName,int automationId) {
		///for improving security[28120201126]:@{
		if(!Account.accountIsExists(accountId, accountName)){
			return "{\"AllConditionsList\":[ ]}";
		}
		///for improving security[28120201126]:@}
	 ...
	 */
	public String getAllActionsAsList(int accountId,String accountName,int automationId) {
		///for improving security[28120201126]:@{
		if(!Account.accountIsExists(accountId, accountName)){
			return "{\"AllActionsList\":[ ]}";
		}
		///for improving security[28120201126]:@}
		String query_sql = "SELECT *FROM tblAutomationOperations WHERE AutomationId='" + automationId +"'";
		Database database = new Database();
...
///REMOVE-PROCESS:@{
	 ...
	 */
	public String removeAutomation(int accountId,String accountName,int automationId) {
		...
		try {
			...
			if(s > 0) {
				...
				String[] tables= {"tblAutomationCondition","tblAutomationOperations"};
	 ...
	 */
	public String removeCondition(int accountId,String accountName,int conditionId) {
	 ...
	 */
	public String removeAction(int accountId,String accountName,int actionId) {
		...
		///for improving security[28120201126]:@}
		String delete_sql = "DELETE FROM tblAutomationOperations WHERE Id='" + actionId +"'";
		Database database = new Database();
		try {
			database.connect();
			int s = database.execSQL(delete_sql);
	...
	/**
	 * Perform automation. Otomasyounu gerçekleştir-işlet(tetikle).(Bu süreç Server yazılımı tarafından "mqtt işlet motoru" olarak oluşturulmuş modül tarafından işletilecek.)
	 * @param _accountId	//which account.for improving security
	 * @param _accountName	//account(email_or_phone)
	 * @param _automationId	//which automation
	 * @return				//return true(as JSON) if successful
	 */
	public String performAutomation(int _accountId,String _accountName,int _automationId) {
...
/////INTERNAL FUNCTIONS:@{
	/**
	 * Activate OR Deactivate out automation. activate or deactivate.
	 * @param automationId	//which automation
	 * @param state			//state activate=1,OR deactivate = 0
	 * @return				//return 1 if successful.
	 */
	private int setActivateOrDeactivateAutomation(int automationId,int state) {
		...
		return s;
	}//setActivateOrDeactivateAutomation()
/////INTERNAL FUNCTIONS:@}
}
