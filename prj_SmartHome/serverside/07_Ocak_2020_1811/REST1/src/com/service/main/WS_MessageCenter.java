...
public class WS_MessageCenter {
	...
	@GET
	@Produces("application/json")
	@Path("/EditMessage/{MessageId}/{Message}/{MessageDateTime}/{MessageType}")
	public Response EditMessage(@PathParam("MessageId")int MessageId,
								@PathParam("Message")String Message,
								@PathParam("MessageDateTime")String MessageDatetime,
								@PathParam("MessageType")int MessageType) {
		/*
		 ...
		 */
		MessageCenter messageCenter = new MessageCenter();
		String response = messageCenter.editMessage(MessageId, Message, MessageDatetime, MessageType);
		return Response.status(200).entity(response).build();
	}
	...
	@GET
	@Produces("application/json")
	@Path("/GetItem/{MessageId}")
	public Response GetItem(@PathParam("MessageId")int MessageId) {
		/*
		 * http://localhost:8080/REST1/MessageCenter/GetItem/1
		 */
		MessageCenter messageCenter = new MessageCenter();
		String response = messageCenter.getItem(MessageId);
		return Response.status(200).entity(response).build();
	}
	...
	public Response RemoveMessage(@PathParam("MessageId")int MessageId) {
		/*
		 * http://localhost:8080/REST1/MessageCenter/RemoveMessage/2
		 */
		MessageCenter messageCenter = new MessageCenter();
		String response = messageCenter.removeMessage(MessageId);
		return Response.status(200).entity(response).build();
	}
