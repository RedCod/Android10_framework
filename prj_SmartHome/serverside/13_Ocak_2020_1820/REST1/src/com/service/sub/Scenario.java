...
CREATE TABLE IF NOT EXISTS tblScenarioSub (
	...
	public String addScenarioAction(int accountId,
		...
		String insert_sql = "INSERT INTO tblScenarioSub(AccountId,ScenarioId,ActionType,DeviceId,DeviceSwitch,AutomationId,TimeLapseValue,Command,ItemSort)"
				+"VALUES('" +accountId+"','"+scenarioId+"','"+actionType+"','"+deviceId+"','"+deviceSwitch+"','"+automationId+"','"+timeLapseValue+"','"+ command +"','"+itemSort+"')";
		Database database = new Database();
		try {
			database.connect();
			int s = database.execSQL(insert_sql);
	 ...
	 * @param command 			//command to run if eit device. "/hesapbilgisi/cihazmacadres_veya_cihazvirtualadres/ON(device switch)"
	...
	public String editScenarioAction(int actionId,
		...
		String sql = "UPDATE tblScenarioSub SET ActionType='" + actionType+"',DeviceId='"+deviceId+"',DeviceSwitch='"+deviceSwitch+"',AutomationId='"+automationId+"',TimeLapseValue='"+timeLapseValue+"',Command='"+command +"',ItemSort='"+itemSort+"' WHERE Id='"+actionId+"'";
		Database database = new Database();
		try {
			database.connect();
			int s = database.execSQL(sql);
	...
	public String performScenario(int scenarioId) {
		...
		String getScenarioActionsAsJSON = getScenarioActions(scenarioId);//get as JSON.
    	try {
            JSONObject jsonObj = new JSONObject(getScenarioActionsAsJSON);
            JSONArray jsonArr = jsonObj.getJSONArray("AllScenarioActionsList");
