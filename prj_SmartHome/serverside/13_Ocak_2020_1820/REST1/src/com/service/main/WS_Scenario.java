...
public class WS_Scenario {
	...
	@GET
	@Produces("application/json")
	@Path("/AddScenarioAction/{AccountId}/{ScenarioId}/{ActionType}/{DeviceId}/{DeviceSwitch}/{AutomationId}/{TimeLapseValue}/{Command}/{ItemSort}")
	 ...
	 * @param Command 		//command to run if edit device. "/hesapbilgisi/cihazmacadres_veya_cihazvirtualadres/ON(device switch)"
	...
	@GET
	@Produces("application/json")
	@Path("/EditScenarioAction/{ActionId}/{ActionType}/{DeviceId}/{DeviceSwitch}/{AutomationId}/{TimeLapseValue}/{Command}/{ItemSort}")
