...
CREATE TABLE IF NOT EXISTS tblFamilyMember (
		...
		synchronized(this) {//Tek tek eriş:tblFamily'e eklenen son kayıta ait Id değeri alını ve tblFamilyMember'de kullanılır.
			...
			try {
				...
				if(s > 0) {
					 ...
					 * Hesap ilk olarak oluşturulurken bir Aile oluşturulması istenir.
	 ...
	 */
	public String editName(int familyId,String familyName) {
		String response = "{\"EditName\":[{ \"response\":false}]}";
		Database database = new Database();
		try {
			String update_sql = "UPDATE tblFamily SET FamilyName='" + familyName + "' WHERE Id='" + familyId +"'";
			database.connect();
	 ...
	 */
	public String editRooms(int familyId,String roomsIds) {
		String response = "{\"EditRooms\":[{ \"response\":false}]}";
		Database database = new Database();
		try {
			String update_sql = "UPDATE tblFamily SET RoomsId='" + roomsIds + "' WHERE Id='" + familyId + "'";
			database.connect();
			int s = database.execSQL(update_sql);
			if(s > 0)
	 ...
	 */
	public String editLocation(int familyId,String familyLocation) {
		String response = "{\"EditLocation\":[{ \"response\":false}]}";
		Database database = new Database();
		try {
			String update_sql = "UPDATE tblFamily SET FamilyLocation='" + familyLocation + "' WHERE Id='" + familyId + "'";
			database.connect();
			int s = database.execSQL(update_sql);
			if(s > 0)
	...
	public String getItem(int id) {
		String query_sql = "SELECT *FROM tblFamily WHERE Id='" + id + "'";
		Database database = new Database();
		String content = "{\"Item\":[\n";
		try {
			database.connect();
	 ...
	 */
	public String removeFamily(int familyId) {
		String response = "{\"RemoveFamily\":[{ \"response\":false}]}";
		Database database = new Database();
		try {
			String delete_sql = "DELETE FROM tblFamily WHERE Id='" + familyId + "'";
			database.connect();
			...
			if(s > 0) {
				...
				delete_sql = "DELETE FROM tblFamilyMember WHERE FamilyId='" + familyId + "'";
				s = database.execSQL(delete_sql);
				if(Log.DEBUG)
					Log.println(TAG, "removeFamily: delete from tblFamilyMember state:" + s);
				//ayrıca eklenen cihazları da sıfırla:
				delete_sql = "DELETE FROM tblDevice WHERE FamilyId='" + familyId + "'";
				s = database.execSQL(delete_sql);
				if(Log.DEBUG)
					Log.println(TAG, "removeFamily: delete from tblDevice state:" + s);
			}
			if(Log.DEBUG)
				Log.println(TAG, "removeFamily: delete on tblFamily state:" + s);
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
		} catch (SQLException e) {
			e.printStackTrace();
