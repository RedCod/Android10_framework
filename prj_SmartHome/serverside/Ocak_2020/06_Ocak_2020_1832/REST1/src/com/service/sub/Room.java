...
CREATE TABLE IF NOT EXISTS tblRoom (
	 ...
	 */
	public String editRoomName(int roomId,String roomName) {
		String response = "{\"EditRoomName\":[{ \"response\":false}]}";
		String sql = "UPDATE tblRoom SET RoomName='" + roomName + "' WHERE Id='" + roomId + "'";
		Database database = new Database();
		try {
			database.connect();
			int s = database.execSQL(sql);
	 ...
	 */
	public String editRoomSort(int roomId,int sortValue) {
		String response = "{\"EditRoomSort\":[{ \"response\":false}]}";
		String sql = "UPDATE tblRoom SET ItemSort='" + sortValue + "' WHERE Id='" + roomId + "'";
		Database database = new Database();
		try {
			database.connect();
			int s = database.execSQL(sql);
	...
	public String getItem(int id) {
		String query_sql = "SELECT *FROM tblRoom WHERE Id='" + id + "'";
		Database database = new Database();
		String content = "{\"Item\":[\n";
		try {
			database.connect();
	...
	public String removeRoom(int roomId) {
		...
		String sql = "DELETE FROM tblRoom WHERE Id='" + roomId + "'";
		Database database = new Database();
		try {
			database.connect();
			int s = database.execSQL(sql);
