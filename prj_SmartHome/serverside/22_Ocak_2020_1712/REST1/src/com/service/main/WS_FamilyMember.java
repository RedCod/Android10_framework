...
public class WS_FamilyMember {
	...
	@GET
	@Produces("application/json")
	@Path("/SetFamilyAdmin/{AccountId}/{FamilyId}")
	public Response SetFamilyAdmin(@PathParam("AccountId")int AccountId,
								   @PathParam("FamilyId")int FamilyId) {
		/*
		 * http://localhost:8080/REST1/FamilyMember/SetFamilyAdmin/1/5
		 */
		FamilyMember family = new FamilyMember();
		String response  = family.setFamilyAdmin(AccountId,FamilyId);
		return Response.status(200).entity(response).build();
	}
	...
	@GET
	@Produces("application/json")
	@Path("/RemoveFamilyAdmin/{AccountId}/{FamilyId}")
	public Response RemoveFamilyAdmin(@PathParam("AccountId")int AccountId,
			                          @PathParam("FamilyId")int FamilyId) {
		/*
		 *  http://localhost:8080/REST1/FamilyMember/RemoveFamilyAdmin/1/5
		 */
		FamilyMember family = new FamilyMember();
		String response = family.removeFamilyAdmin(AccountId,FamilyId);
		return Response.status(200).entity(response).build();
	}
